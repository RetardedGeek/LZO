use crate::helpers::{get_unaligned_32le, get_unaligned_64le, put_unaligned_le32};
use std::cmp::min;
...
pub fn lzo1x_do_compress(
    input: &[u8],
    out: &mut [u8],
    out_ind: &mut usize,
    tp: &mut usize,
    wrkmem: &mut [usize],
    state_offset: &mut isize,
        ...
        'literal: loop {
            ...
            'next: loop {
                ...
                'outer: loop {
                    ...
                    {
                        let mut v: u64 = get_unaligned_64le(input, ip + m_len)
                            ^ get_unaligned_64le(input, m_pos + m_len);
                        if v == 0 {
                            loop {
                                m_len += 8;
                                v = get_unaligned_64le(input, ip + m_len)
                                    ^ get_unaligned_64le(input, m_pos + m_len);
                                if ip + m_len >= ip_end {
                                    break 'outer;
                                }
                    ...
                    #[cfg(all(
                        not(all(feature = "efficient_unaligned_access", feature = "ctz64")),
                        feature = "efficient_unaligned_access",
                        feature = "ctz32"
                    ))]
                    {
...
pub fn lzogeneric1x_1_compress(
    input: &[u8],
    in_len: usize,
    out: &mut [u8],
    out_len: &mut usize,
