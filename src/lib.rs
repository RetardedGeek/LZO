...
fn lzo1x_do_compress(input :&[u8],out :&mut [u8],out_ind :&mut usize,tp :&mut usize,wrkmem :&mut[usize],state_offset :&mut isize,bitstream_version :u8) -> Result<(),Error>
{
    ...
    'm: loop
    {
       ...
       'next: loop{
        ...
        {
            t=(((dv * 0x1824429d) >> (32-D_BITS)) & D_MASK) as usize;
            m_pos = dict[t];
            dict[t as usize]=ip;
